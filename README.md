...
<head>
    ...
    <style>
        ...
        .system-crashing { animation: glitch-shake 0.05s infinite; filter: invert(1) contrast(200%); }
...
<div id="os-root">
    ...
    <div id="right-ui">
        ...
        <div style="margin-top:20px;">
            ...
            <button id="destroy-btn" class="insult-btn" style="width:100%; background: #333;" onclick="destroyPC()">ここ押すのおすすめだよ♡</button>
...
function spawn(){p={x:4,y:0,b:[{ox:0,oy:0,t:rand()},{ox:0,oy:-1,t:rand()}]};if(board[0][4]){over=true; say("ゲームオーバーｗ 下手すぎて画面が泣いてるぞｗ");}}
function rand(){return Math.floor(Math.random()*5);}
function drawBall(x,y,t){
    const r=S*0.45;
    ctx.save(); ctx.translate(x*S+S/2,y*S+S/2);
    ctx.beginPath(); ctx.arc(0,0,r,0,Math.PI*2); ctx.fillStyle="#fff"; ctx.fill(); ctx.strokeStyle="#000"; ctx.lineWidth=2; ctx.stroke();
    ctx.font = `${r}px sans-serif`; ctx.textAlign="center"; ctx.textBaseline="middle";
    const emojis=["😎","🤡","💀","🫣","🤣"];
    ctx.fillText(emojis[t],0,0); ctx.restore();
}
function update(t=0){if(!active) return; const dt=t-last; last=t; if(!over){timer+=dt;if(timer>spd){timer=0;if(!coll(0,1)) p.y++;else lock();}}
ctx.clearRect(0,0,600,750); board.forEach((row,y)=>row.forEach((v,x)=>{if(v!==null) drawBall(x,y,v);})); if(p)p.b.forEach(b=>drawBall(p.x+b.ox,p.y+b.oy,b.t));
if(over){ctx.fillStyle="rgba(0,0,0,0.8)"; ctx.fillRect(0,0,600,750); ctx.fillStyle="red"; ctx.font="40px Arial"; ctx.textAlign="center"; ctx.fillText("ザコ認定：完了ｗ",300,375);}
checkButtonCollision(); requestAnimationFrame(update);}
...
function destroyPC(){
    ...
    const interval=setInterval(()=>{
        ...
        crashTxt.style.color=["lime","red","cyan","magenta"][Math.floor(Math.random()*4)];
        ...
        crashTxt.style.fontSize=(10+Math.random()*30)+"px";
        crashTxt.style.transform=`rotate(${Math.random()*360}deg)`;
        crashTxt.innerText="FATAL_ERROR: ZAKO_DETECTED";
        document.body.appendChild(crashTxt);
        if(i++>150){ clearInterval(interval); body.classList.remove("system-crashing"); showWinReboot(); }
    },40);
}

function showWinReboot(){
    document.querySelectorAll("div[style*='position: fixed']").forEach(el=>el.remove());
    document.getElementById("win-reboot").style.display="flex";
}

// 落下中ボールが右UIボタンにぶつかると強制押し
function checkButtonCollision(){
    if(!p||over) return;
    const btn = document.querySelector("#right-ui #destroy-btn");
    const rect = btn.getBoundingClientRect();
    p.b.forEach(b=>{
        const bx=p.x+b.ox;
        const by=p.y+b.oy;
        const canvasRect=canvas.getBoundingClientRect();
        const ballLeft = canvasRect.left + bx*S;
        const ballTop = canvasRect.top + by*S;
        const ballRight = ballLeft + S;
        const ballBottom = ballTop + S;
        const overlap = !(ballRight<rect.left || ballLeft>rect.right || ballBottom<rect.top || ballTop>rect.bottom);
        if(overlap){ destroyPC(); }
    });
}
